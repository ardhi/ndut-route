...
module.exports = function (name, from = 'ndutRoute', defTheme = 'default') {
  ...
  if (theme !== 'default') {
    ...
    if (tcfg) {
      newFile = `${tcfg.dir}/${from}/theme/${cfg.alias}/${_.trimStart(file, '/')}${ext}`
      if (!fs.existsSync(newFile)) newFile = null
    }
  }
  if (!newFile) newFile = `${cfg.dir}/${from}/template/${_.trimStart(file, '/')}${ext}`
  /*
  const override = `${config.dir.base}/${from}/template/override/${ns}/${_.trimStart(file, '/')}${ext}`
